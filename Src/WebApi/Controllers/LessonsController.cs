...
public class LessonsController : ControllerBase
{
    ...
    /// <summary>
    /// Get all deleted lessons for a course (Admin only)
    /// </summary>
    [HttpGet("course/{courseId:guid}/deleted")]
    [Authorize(Roles = Roles.Admin)]
    [ProducesResponseType(typeof(IEnumerable<LessonDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    public async Task<ActionResult<IEnumerable<LessonDto>>> GetDeletedByCourseId(Guid courseId)
    {
        var lessons = await _lessonService.GetDeletedByCourseIdAsync(courseId);
        return Ok(lessons);
    }

    /// <summary>
    /// Restore a deleted lesson (Admin only)
    /// </summary>
    ...
    public async Task<IActionResult> Restore(Guid id)
    {
        ...
        catch (CannotRestoreException ex)
        {
            _logger.LogWarning(ex, "Cannot restore lesson: {LessonId}", id);
            return BadRequest(new { message = ex.Message });
        }
    }
